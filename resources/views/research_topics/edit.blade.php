    ...
    <div class="container">
        ...
        <form action="{{ route('research_topics.update', $topic->topic_id) }}" method="POST">
                ...
                <select name="research_direction_id" id="research_direction_id" class="form-select" required>
                    ...
                    @foreach ($researchDirections as $direction)
                        <option value="{{ $direction->research_direction_id }}"
                            ...
                            {{ $direction->research_direction_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="research_goal" id="research_goal" rows="3" class="form-control">{{ $topic->research_goal }}</textarea>
            </div>
            <div class="mb-3">
                <label for="content" class="form-label">Nội dung</label>
                <textarea name="content" id="content" rows="4" class="form-control">{{ $topic->content }}</textarea>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="date" name="end_date" id="end_date" class="form-control" value="{{ $topic->end_date }}">
            </div>
